...
parser.add_argument('-interpolate', '-i', type=float, required=False, help='Linear interpolation alpha (percentage from 0 to 1).') # Option to interpolate values between models. Higher alpha means higher weight from netB model
parser.add_argument('-splice', '-s', type=float, required=False, help='Enable random splice between models.') # Option to randomly transplant filters. Higher alpha means higher probability to take from netB model
...
if args.netA:
    ...
    netA_path = '../../experiments/pretrained_models/RRDB_PSNR_x4.pth' #Default just for tests
...
if args.interpolate:
    ...
    net_interp_path = '../../experiments/pretrained_models/int_{:02d}.pth'.format(int(alpha*10))
    print('Interpolating with alpha = ', alpha) #Higher alpha means higher weight from netB model
...
for k, v_netA in netA.items():
    if k in netB: #for models with different scales, this will automatically work for convolution layers only, upscale layers are different 
        v_netB = netB[k]
        if splice > 0: #random splice enabled, no interpolation 
            #if random.choice([1,0]) == 0: 
            if random.uniform(0., 1.) > splice: 

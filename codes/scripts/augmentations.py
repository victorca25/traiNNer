...
def random_erasing(
    ...
    if mode == 0:  # original code
        mask_value = np.random.uniform(0.0, 1.0)
        image[top:bottom, left:right, :].fill(mask_value)
        ...
        image[top:bottom, left:right, :] = np.random.rand(
...
def random_resize_img(img_LR, crop_size=(128, 128), new_scale=0, algo=None):
    mu, sigma = 0, 0.3
    if new_scale == 0:
        new_scale = np.random.uniform(-0.5, 0.0)  # resize randomly from 0 to 2x
        scale = 1 + new_scale
    else:
        scale = new_scale
...
def random_rotate_pairs(img_HR, img_LR, HR_size, scale, angle=0, center=0):
    ...
    img_HR, img_LR = random_crop_pairs(
        img_HR, img_LR, HR_size, scale
    )  # crop back to the original size if needed
    return img_HR, img_LR
...
def noise_img(img_LR, noise_types=["clean"]):
    ...
    if (
        ...
        flipped = np.random.choice(
            [True, False], size=out.shape, p=[amount, 1 - amount]
        )
        ...
        speckle_type = [
            c,
            1,
        ]  # randomize if colored noise (c) or 1 channel (B&W). #this can be done with other types of noise
        ...
        gauss_type = [
            c,
            1,
        ]  # randomize if colored gaussian noise or 1 channel (B&W). #this can be done with other types of noise
        c = random.choice(gauss_type)

        mean = 0
        sigma = np.random.uniform(4, 200.0)
        gauss = np.random.normal(scale=sigma, size=(h, w, c))
        gauss = gauss.reshape(h, w, c) / 255.0
        ...
        som = MiniSom(
            2, N, 3, sigma=1.0, learning_rate=0.2, neighborhood_function="bubble"
        )  # 2xN final colors
        ...
        if dither_type == "binary":
            ...
            for i in range(0, size[0] - 1):
                for j in range(1, size[1] - 1):
                    ...
                    re_fs[i + 1, j - 1] = minmax(
                        re_fs[i + 1, j - 1] + (3 / 16.0) * quant_error
                    )
                    re_fs[i + 1, j] = minmax(re_fs[i + 1, j] + (5 / 16.0) * quant_error)
                    re_fs[i + 1, j + 1] = minmax(
                        re_fs[i + 1, j + 1] + (1 / 16.0) * quant_error
                    )
        ...
        # Floyd-Steinberg (F-S) Dithering
            ...
            for i in range(0, size[0] - 1):
                for j in range(1, size[1] - 1):
                    ...
                    re_fs[i, j + 1, 0] = minmax(
                        re_fs[i, j + 1, 0] + (7 / 16.0) * quant_error_b
                    )
                    re_fs[i, j + 1, 1] = minmax(
                        re_fs[i, j + 1, 1] + (7 / 16.0) * quant_error_g
                    )
                    re_fs[i, j + 1, 2] = minmax(
                        re_fs[i, j + 1, 2] + (7 / 16.0) * quant_error_r
                    )
                    ...
                    re_fs[i + 1, j, 0] = minmax(
                        re_fs[i + 1, j, 0] + (5 / 16.0) * quant_error_b
                    )
                    re_fs[i + 1, j, 1] = minmax(
                        re_fs[i + 1, j, 1] + (5 / 16.0) * quant_error_g
                    )
                    re_fs[i + 1, j, 2] = minmax(
                        re_fs[i + 1, j, 2] + (5 / 16.0) * quant_error_r
                    )
...
def simplest_cb(img, percent=1, znorm=False):
    ...
    )  # need to add astype(np.uint8) so cv2.LUT doesn't fail later
...
def random_img(
    ...
    # """
    cv2.imwrite(
        save_path + "/scale_" + str(scale) + "_" + str(interpol_algo) + "_.png",
        img_scale * 255,
    )
    # """
    ...
    # """
    cv2.imwrite(
        save_path + "/blur_" + str(blur_kernel_size) + "_" + str(blur_algo) + "_.png",
        img_blur * 255,
    )
    # """
...
def apply_dir(
    ...
    for path in img_list:
        ...
        # """
        cv2.imwrite(
            save_path + "/" + rann + "random_resize_.png", img_random_resize * 255
        )
        # """
        ...
        # """
        cv2.imwrite(
            save_path + "/" + rann + "rrot_" + str(angle) + "_.png", img_rrot * 255
        )
        # """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Select mode. 1: Single Image, 2: Random image from directory, 3: Full directory",
    )
    ...
    parser.add_argument(
        "-noise2", type=str, required=False, help="Secondary noise algorithm"
    )
    ...
    if args.mode == 1:
        single_image(
            img_path, savepath, crop_size, scale, blur_algos, noise_types, noise_types2
        )  # take an image path and apply augmentations
    elif args.mode == 2:
        random_img(
            img_dir, savepath, crop_size, scale, blur_algos, noise_types, noise_types2
        )  # take a random image from the directory and apply augmentations
    elif args.mode == 3:
        apply_dir(
            img_dir, savepath, crop_size, scale, blur_algos, noise_types, noise_types2
        )  # take a random image from the directory and apply augmentations
        ...
        print(
            "Mode not selected, please select one of 1: Single Image, 2: Random image from directory, 3: Full directory"
        )

...
for img_name, write_txt in zip(img_name_list, text_list):
    ...
    if enlarge_ratio > 1:
        H, W, _ = img.shape
        img = cv2.resize(
            img, (W * enlarge_ratio, H * enlarge_ratio), interpolation=cv2.INTER_CUBIC
        )

    # add text
    font = cv2.FONT_HERSHEY_COMPLEX
    cv2.putText(
        img, write_txt, txt_pos, font, font_size, color, font_thickness, cv2.LINE_AA
    )
...
if use_imageio:
    ...
    imageio.mimsave(
        os.path.join(save_folder, "out.gif"), img_list, format="GIF", duration=duration
    )

if make_video:
    os.system(
        "ffmpeg -r {:f} -i {:s}/%05d.png -vcodec mpeg4 -y {:s}/movie.mp4".format(

...
class SelfAttentionBlock(nn.Module):
    ...
    def __init__(self,in_dim,max_pool=False,poolsize = 4,spectral_norm=True,ret_attention=False): #in_dim = in_feature_maps
        ...
        if self.max_pool: #Upscale to original size
            self.upsample_o = Upsample(scale_factor=self.poolsize, mode='bilinear', align_corners=False) #bicubic (PyTorch > 1.0) | bilinear others.
            # Note: test using strided convolutions instead of MaxPool2d! :
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1, spectral_norm = True):
...
class UpscaleBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None, kernel_size=3, stride=1, upscale_factor=2):
...
class UpconvBlock(nn.Module):
    ...
    '''
    # Implements resize-convolution
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, upscale_factor=2, bias=True, \
        ...
        self.upsample = B.Upsample(scale_factor=upscale_factor, mode=mode) #Updated to prevent the "nn.Upsample is deprecated" Warning
        self.conv = B.conv_block(in_channels, out_channels, kernel_size, stride, bias=bias, \
...
class ASRResNet(nn.Module):
    def __init__(self, scale_factor=4, spectral_norm = True, self_attention = True, max_pool=False, poolsize = 4):
    ...
    def forward(self, x, isTest=False, outm=None):
        ...
        if isTest == True:
            origin_size = x.size()
            input_size  = (math.ceil(origin_size[2]/self.scale_factor)*self.scale_factor, math.ceil(origin_size[3]/self.scale_factor)*self.scale_factor)
            out_size    = (origin_size[2]*self.scale_factor, origin_size[3]*self.scale_factor)
...
class ADiscriminator(nn.Module):
    def __init__(self, spectral_norm = True, self_attention = True, max_pool=False, poolsize = 4):
        ...
        if self.self_attention:
            self.FSA = SelfAttentionBlock(in_dim = 256, max_pool=self.max_pool, poolsize = self.poolsize, spectral_norm=self.spectral_norm)
        ...
        self.conv10 = nn.Conv2d(1024, 1, kernel_size=1) # should be equivalent to self.classifier?

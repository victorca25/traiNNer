...
from dataops.filters import get_gaussian_kernel2d, get_kernel_size


class ImRes():
    def __init__(self, im_shape=(100, 100), scale_factor=None, output_shape=None, kernel=None, align_center=False, alg='cv2'):
        scale_factor = self.val_scale(im_shape, scale_factor, output_shape)
        self.alg = alg
        self.kernels = {}
        self.update_kernels(kernel=kernel, scale_factor=scale_factor, align_center=align_center)
    ...
    def update_kernels(self, kernel, scale_factor, im_shape=None, output_shape=None, align_center=False):
        kernel_types = ['cubic', 'blurry_cubic', 'linear', 'lanczos', 'reset_2_default']
        assert kernel is None or isinstance(kernel,np.ndarray) or any([word in kernel for word in kernel_types])
        ...
        self.pre_stride, self.post_stride = calc_strides(im_shape=im_shape, factor=scale_factor, align_center=align_center)
        scale_factor = self.val_scale(im_shape, scale_factor, output_shape)
        sf_4_kernel = np.maximum(scale_factor, 1 / scale_factor).astype(np.int32)  # scale factor for kernel, either upscale or downscale
        ...
        if isinstance(kernel, np.ndarray):
            ...
            if str(sf_4_kernel) in self.kernels.keys(): #Called by 'reset_2_default'
                ...
                kernel = 'cubic'
            kernel_2_use = get_interpolation_kernel(sf_4_kernel, kernel=kernel, alg=self.alg)
        ...
        antialiasing_kernel = np.pad(self.kernels[str(sf_4_kernel)], ((kernel_pre_padding[0], kernel_post_padding[0]), (kernel_pre_padding[1], kernel_post_padding[1])), mode='constant')
        if scale_factor < 1:
    ...
    def resize(self, im, scale_factor=None, output_shape=None, use_zero_padding=False):
        ...
        padding_size = np.floor(np.array(self.antialiasing_kernel.shape)/2).astype(np.int32)
        ...
        def filter2d(input, special_padding_size=None):
            ...
            if self.antialiasing_kernel.size > 1000:
                print('Using GPU for image resizing (since kernel is of size %dx%d)' % (self.antialiasing_kernel.shape[0], self.antialiasing_kernel.shape[1]))
                return torch.nn.functional.conv2d(torch.from_numpy(input).cuda().unsqueeze(0).unsqueeze(0),
                    torch.from_numpy(1 * np.rot90(self.antialiasing_kernel.astype(input.dtype), 2)).unsqueeze(0).unsqueeze(0).cuda(),
                    padding=(self.antialiasing_kernel.shape[0] // 2, self.antialiasing_kernel.shape[1] // 2) if special_padding_size is None else 0).squeeze(0).squeeze(0).cpu().numpy()
            else:
                return convolve2d(input, self.antialiasing_kernel, 'same' if special_padding_size is None else 'valid')
        
        for channel_num in range(im.shape[2]):
            if scale_factor > 1:  # Upscale
                # expand the image by stuffing zeros according
                output.append(np.reshape(np.pad(np.expand_dims(
                                np.expand_dims(im[:,:,channel_num], 2), 1), 
                                    ((0,0), (self.pre_stride[0], self.post_stride[0]), 
                                    (0,0), (self.pre_stride[1], self.post_stride[1])),
                ...
                output[-1] = output[-1][self.pre_stride[0]::int(1 / scale_factor), self.pre_stride[1]::int(1 / scale_factor)]
...
def calc_strides(im=None, factor=None, align_center=False, im_shape=None):
    ...
    if align_center:
        if isinstance(im, np.ndarray):
            im_shape = im.shape[:2]
        assert im_shape
        half_image_size = np.ceil(np.array(im_shape)/2*(factor if factor > 1 else 1))
...
def get_interpolation_kernel(sf, delta_size: int = 11, kernel: str ='cubic', alg='cv2'):
    ...
    if alg == 'torch':
        upscale_kernel = torch.nn.functional.interpolate(torch.from_numpy(delta_im).unsqueeze(0).unsqueeze(0),
                            scale_factor=sf, mode=mode, align_corners=False ).squeeze(0).squeeze(0).numpy()
    else:
        upscale_kernel = cv2.resize(delta_im, dsize=(sf*delta_size, sf*delta_size), interpolation=mode)

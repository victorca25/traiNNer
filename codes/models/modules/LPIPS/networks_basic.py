...
def spatial_average(in_tens, keepdim=True):
    ...
    return torch.mean(torch.mean(in_tens, dim=3), dim=2)[0][
        0
    ]  # For any Pytorch version. alt: "in_tens.mean(dim=3).mean(dim=2)[0][0]" #Needs to pick the first element of each dimension list
...
def upsample(in_tens, out_H=64):  # assumes scale factor is same for H and W
    ...
    return Upsample(scale_factor=scale_factor, mode="bilinear", align_corners=False)(
        in_tens
    )

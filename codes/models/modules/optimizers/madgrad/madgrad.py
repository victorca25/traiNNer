...
class MADGRAD(Optimizer):
    ...
    def __init__(self, params: _params_t, lr: float = 1e-2,
                momentum: float = 0.9, eps: float = 1e-6,
                weight_decay: float = 0, decay_type='Adam'):
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Apply weight decay
                if decay != 0:
                    if self.decay_type == "AdamW":
                        # apply L2 / AdamW style weight decay
                        p.data.mul_(1 - lr * decay)
                    else:
                        # original Adam style weight decay
                        if grad.is_sparse:
                            raise RuntimeError(
                                "weight_decay option is not compatible with sparse gradients, try using AdamW decay_type")

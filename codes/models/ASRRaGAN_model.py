...
class ASRRaGANModel(BaseModel):
        ...
        if self.is_train:
            ...
            if train_opt["gan_weight"]:
                if train_opt["gan_type"] == "basic":
                    ...
                    self.D_update_ratio = (
                        train_opt["D_update_ratio"]
                        if train_opt["D_update_ratio"]
                        else 1
                    )
                    self.D_init_iters = (
                    ...
                    self.cri_gan = GANLoss(train_opt["gan_type"], 1.0, 0.0).to(
                        self.device
                    )
                    self.l_gan_w = train_opt["gan_weight"]
    ...
    def optimize_parameters(self, step):
        ...
        if self.cri_gan:
            ...
            pred_d_real, feats_d_real = self.netD(
                self.var_ref
            )  # original with Feature maps
            # pred_d_fake = self.netD(self.fake_H.detach())  # detach to avoid BP to G # original
            pred_d_fake, feats_d_fake = self.netD(
                self.fake_H.detach()
            )  # detach to avoid BP to G # original with Feature maps
            ...
            if step % self.D_update_ratio == 0 and step > self.D_init_iters:
                ...
                if (
                    ...
                    for hr_feat_map, sr_feat_map in zip(feats_d_fake, feats_d_real):
                        ...
                        l_g_disfea += self.l_disfea_w * self.cri_disfea(
                            sr_feat_map, hr_feat_map
                        )
            ...
            if self.cri_ssim:  # structural loss (Structural Dissimilarity)
                l_g_ssim = (
                    1.0 - (self.l_ssim_w * self.cri_ssim(self.fake_H, self.var_H))
                ) / 2  # using ssim2.py
                if torch.isnan(
                    l_g_ssim

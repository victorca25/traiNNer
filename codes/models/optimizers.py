...
def get_optimizers(cri_gan=None, netD=None, netG=None, train_opt=None, logger=None, optimizers=None):
    ...
    if cri_gan:
        ...
        if optim_D == 'sgd':
            optimizer_D = torch.optim.SGD(netD.parameters(), lr=lr_D,
            ...
            optimizer_D = MADGRAD(optim_params, lr=lr_D,
                eps=eps_D, momentum=momentum_D, weight_decay=wd_D,
                decay_type=train_opt.get('decay_type_D', 'AdamW'))
        else: # default to 'adam'
            optimizer_D = torch.optim.Adam(netD.parameters(), lr=lr_D,
                weight_decay=wd_D, betas=(beta1_D, beta2_D))

...
class Model:
    ...
    def save_network(self, network, network_label, iter_step):
        ...
        torch.save(state_dict, os.path.join(self.opt["path"]["models"], f"{iter_step}_{network_label}.pth"))
    ...
    def save_training_state(self, epoch, iter_step):
        ...
        }, os.path.join(self.opt["path"]["training_state"], f"{iter_step}.state"))

    def resume_training(self, resume_state):
        """Resume the optimizers and schedulers for training"""
        resume_optimizers = resume_state["optimizers"]
        resume_schedulers = resume_state["schedulers"]
        if not len(resume_optimizers) == len(self.optimizers):
            raise ValueError("Wrong lengths of optimizers")
        if not len(resume_schedulers) == len(self.schedulers):
            raise ValueError("Wrong lengths of schedulers")
    ...
    def update_schedulers(self, train_opt):
        ...
        if train_opt["lr_scheme"] == "StepLR":
            for i, s in enumerate(self.schedulers):
                if self.schedulers[i].step_size != train_opt["lr_step_size"] and train_opt["lr_step_size"] is not None:
                    print(f"Updating step_size from {self.schedulers[i].step_size} to {train_opt['lr_step_size']}")
                ...
                if self.schedulers[i].gamma != train_opt["lr_gamma"] and train_opt["lr_gamma"] is not None:
                    print(f"Updating lr_gamma from {self.schedulers[i].gamma} to {train_opt['lr_gamma']}")
                    self.schedulers[i].gamma = train_opt["lr_gamma"]
        if train_opt["lr_scheme"] == "StepLR_Restart":
            for i, s in enumerate(self.schedulers):
                if self.schedulers[i].step_sizes != train_opt["lr_step_sizes"] and \
                        train_opt["lr_step_sizes"] is not None:
                    print(f"Updating step_sizes from {self.schedulers[i].step_sizes} to {train_opt['lr_step_sizes']}")
                ...
                if self.schedulers[i].restarts != train_opt["restarts"] and train_opt["restarts"] is not None:
                    print(f"Updating restarts from {self.schedulers[i].restarts} to {train_opt['restarts']}")
                    self.schedulers[i].restarts = train_opt["restarts"]
                if self.schedulers[i].restart_weights != train_opt["restart_weights"] and \
                        train_opt["restart_weights"] is not None:
                    print(f"Updating restart_weights from {self.schedulers[i].restart_weights} to "
                          f"{train_opt['restart_weights']}")
                ...
                if self.schedulers[i].clear_state != train_opt["clear_state"] and train_opt["clear_state"] is not None:
                    print(f"Updating clear_state from {self.schedulers[i].clear_state} to {train_opt['clear_state']}")
            ...
            for i, s in enumerate(self.schedulers):
                if self.schedulers[i].milestones != train_opt["lr_steps"] and train_opt["lr_steps"] is not None:
                    if not list(train_opt["lr_steps"]) == sorted(train_opt["lr_steps"]):
                        raise ValueError("lr_steps should be a list of increasing integers. "
                                         f"Got {train_opt['lr_steps']}")
                    print(f"Updating lr_steps from {list(self.schedulers[i].milestones)} to {train_opt['lr_steps']}")
        ...
        if train_opt["lr_scheme"] == "MultiStepLR_Restart":
            for i, s in enumerate(self.schedulers):
                if self.schedulers[i].milestones != train_opt["lr_steps"] and train_opt["lr_steps"] is not None:
                    if not list(train_opt["lr_steps"]) == sorted(train_opt["lr_steps"]):
                        raise ValueError(f"lr_steps should be a list of increasing integers. "
                                         f"Got {train_opt['lr_steps']}")
                    print(f"Updating lr_steps from {list(self.schedulers[i].milestones)} to {train_opt['lr_steps']}")
                    if isinstance(self.schedulers[i].milestones, Counter):
                        self.schedulers[i].milestones = Counter(train_opt["lr_steps"])
                ...
                if self.schedulers[i].restart_weights != train_opt["restart_weights"] and \
                        train_opt["restart_weights"] is not None:
                    print(f"Updating restart_weights from {self.schedulers[i].restart_weights} "
                          f"to {train_opt['restart_weights']}")
                    self.schedulers[i].restart_weights = train_opt["restart_weights"]
                if self.schedulers[i].clear_state != train_opt["clear_state"] and train_opt["clear_state"] is not None:
                    print(f"Updating clear_state from {self.schedulers[i].clear_state} to {train_opt['clear_state']}")
                    self.schedulers[i].clear_state = train_opt["clear_state"]

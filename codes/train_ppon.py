    ...
    if opt["path"]["resume_state"]:
        if os.path.isdir(opt["path"]["resume_state"]):
            ...
            resume_state_path = util.sorted_nicely(
                glob.glob(os.path.normpath(opt["path"]["resume_state"]) + "/*.state")
            )[-1]
        else:
        ...
        for n, train_data in enumerate(train_loader, start=1):
            ...
            if current_step % opt["logger"]["save_checkpoint_freq"] == 0:
                ...
                model.save_training_state(
                    epoch + (n >= len(train_loader)), current_step
                )
            ...
            if current_step % opt["train"]["val_freq"] == 0:
                ...
                for val_data in val_loader:
                    ...
                    if opt["datasets"]["train"][
                        ...
                        img_c = util.tensor2img(
                            visuals["img_c"], min_max=(-1, 1)
                        )  # uint8
                        img_s = util.tensor2img(
                            visuals["img_s"], min_max=(-1, 1)
                        )  # uint8
                        img_p = util.tensor2img(
                            visuals["img_p"], min_max=(-1, 1)
                        )  # uint8
                        gt_img = util.tensor2img(
                            visuals["HR"], min_max=(-1, 1)
                        )  # uint8
                    ...
                    save_c_img_path = os.path.join(
                        img_dir, "{:s}_{:d}_c.png".format(img_name, current_step)
                    )
                    save_s_img_path = os.path.join(
                        img_dir, "{:s}_{:d}_s.png".format(img_name, current_step)
                    )
                    save_p_img_path = os.path.join(
                        img_dir, "{:s}_{:d}_d.png".format(img_name, current_step)
                    )
                    ...
                    sr_img_c = img_c / 255.0  # C
                    sr_img_s = img_s / 255.0  # S
                    sr_img_p = img_p / 255.0  # D
                    ...
                    avg_ssim_c += util.calculate_ssim(
                        cropped_sr_img_c * 255, cropped_gt_img * 255
                    )

                    avg_psnr_s += util.calculate_psnr(
                        cropped_sr_img_s * 255, cropped_gt_img * 255
                    )
                    avg_ssim_s += util.calculate_ssim(
                        cropped_sr_img_s * 255, cropped_gt_img * 255
                    )

                    avg_psnr_p += util.calculate_psnr(
                        cropped_sr_img_p * 255, cropped_gt_img * 255
                    )
                    avg_ssim_p += util.calculate_ssim(
                        cropped_sr_img_p * 255, cropped_gt_img * 255
                    )
                ...
                logger_val.info(
                    "<epoch:{:3d}, iter:{:8,d}>".format(epoch, current_step)
                )
                logger_val.info(

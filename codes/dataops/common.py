...
def bgr2ycbcr(image: np.ndarray, only_y=True, separate=False):
    """
    BGR version of MATLAB RGB2YCbCr.
    Python opencv library (cv2) cv2.COLOR_BGR2YCrCb has different parameters with MATLAB color conversion.
    Input:
        uint8, [0, 255]
        float, [0, 1]
    :param image: input image to convert
    :param only_y: only return Y channel
    :param separate: return the channels as separate images
    """
    in_img_type = image.dtype
    img_as_float = image.astype(np.float32)
    if in_img_type != np.uint8:
        img_as_float *= 255.
    # convert
    if only_y:
        result = np.dot(img_as_float, [24.966, 128.553, 65.481]) / 255.0 + 16.0
    else:
        result = np.matmul(
            img_as_float,
            [
...
def np2tensor(img, bgr2rgb=True, data_range=1., normalize=False, change_range=True, add_batch=True):
    ...
    img = torch.from_numpy(
        np.ascontiguousarray(np.transpose(img, (2, 0, 1)))).float()  # "HWC to CHW" and "numpy to tensor"
    if bgr2rgb:
...
def recompose_tensor(patches, full_height, full_width, overlap=10):
    ...
    for h in range(n_patches_height):
        for w in range(n_patches_width):
            ...
            blending_image[0, :, patch_start_height: patch_start_height + patch_size,
            patch_start_width: patch_start_width + patch_size] += blending_patch[None]
    ...
    for b in range(final_batch_size):
        for h in range(n_patches_height):
            for w in range(n_patches_width):
                patch_start_height = min(h * effective_patch_size, full_height - patch_size)
                patch_start_width = min(w * effective_patch_size, full_width - patch_size)
                recomposed_tensor[b, :, patch_start_height: patch_start_height + patch_size,
                patch_start_width: patch_start_width + patch_size] += patches[patch_index] * blending_patch
                patch_index += 1

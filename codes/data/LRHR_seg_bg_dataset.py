...
class LRHRSeg_BG_Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.HR_env_bg, self.paths_HR_bg = util.get_image_paths(
            opt["data_type"], opt["dataroot_HR_bg"]
        )
        ...
        self.ratio = (
            10  # 10 OST data samples and 1 DIV2K general data samples(background)
        )
    ...
    def __getitem__(self, index):
        ...
        if self.paths_LR:
            ...
            if self.opt["phase"] == "train":
                ...
                seg = cv2.resize(
                    np.copy(seg), (W_s, H_s), interpolation=cv2.INTER_NEAREST
                )
        ...
        if self.opt["phase"] == "train":
            ...
            img_LR, img_HR, seg = util.augment(
                [img_LR, img_HR, seg], self.opt["use_flip"], self.opt["use_rot"]
            )
        ...
        seg = torch.from_numpy(
            np.ascontiguousarray(np.transpose(seg, (2, 0, 1)))
        ).float()
